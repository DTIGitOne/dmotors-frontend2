...
import CardCarMonitor from '../Components/CardCarMonitor';
...
const SearchResults = () => {
    ...
    return (
        ...
        <div className='w-full flex justify-center'>
            {resultLoading ? (
                <div className="bg-white h-screen w-screen fixed top-0 left-0 flex justify-center items-center" style={{ zIndex: "9999" }}>
                    <LoaderIcon />
                </div>
            ) : (null)}
            <div id='searchResultBox' className='bg-slate-100 w-full h-auto pt-10 flex flex-col items-center'>
                ...
                <div id='divScreenResult' className=' flex justify-center'>
                <div id='windowSearch' className=' p-4 flex flex-col gap-5 items-center bg-white h-auto rounded-2xl' style={{minWidth: "20%", color: "#534D56" }}>
                    <span className="text-2xl font-medium">Change search</span>
                    <CarTypeInput />
                </div>
                <div className="search-results-container flex flex-col gap-3">
                    {cars.length > 0 ? (
                        cars.map((car) => (
                            isLargeScreen ? (
                                <CardCarMonitor
                        ...
                        <div id='noCarsFound' className=' h-full w-full text-2xl text-center flex justify-center items-center'>No cars found.</div>
                    )}
                </div>
                </div>
